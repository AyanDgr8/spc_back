...
// 2. Mail transport -------------------------------------------------
// Primary configuration for Office365 (commented out)
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: process.env.EMAIL_USER || 'ayan@multycomm.com',
    pass: process.env.EMAIL_PASSWORD || 'snwb pexk avoq lnyl', // Gmail app password
  },
  debug: true, // Enable debug output
  logger: true // Log to console
});
...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.log("- User:", process.env.EMAIL_USER || 'ayan@multycomm.com');
    console.log("- Password length:", (process.env.EMAIL_PASSWORD || 'snwb pexk avoq lnyl').length);
    console.log("\n🔧 Troubleshooting suggestions:");
    console.log("1. Verify the Gmail account exists and is active");
    ...
    console.log("5. Check Gmail SMTP settings");
  } else {
