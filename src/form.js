...
export async function handleFormSubmission(data) {
  ...
  await pool.execute(sql, [
    company, name, contact_number, email, call_type, disposition_1, disposition_2,
    disposition_2_custom || null, query || null, queue_id || null, queue_name || null, 
  ...
  const disposition2Display = isCustomInput && disposition_2_custom 
    ? disposition_2_custom 
    : disposition_2;
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <br/>
      <p><strong>Ticket raised for:</strong></p>
      ...
      <br/>
      <p><em>{Notes updated from Multycomm}</em></p>
      <br/>
...
export async function updateFormSubmission(id, data) {
  ...
  const {
    ...
    disposition_2_custom = current.disposition_2_custom,
    query = current.query,
  ...
  const sql = `UPDATE forms_new SET 
    ...
    disposition_2_custom = ?, query = ?, queue_id = ?, queue_name = ?,
